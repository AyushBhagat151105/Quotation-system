...
export class QuotationService {
  ...
  async create(dto: CreateQuotationDto, adminId: string) {
    try {
      ...
      const totalAmount = itemsData
        .reduce(
          (acc, it) => acc.plus(new Decimal(it.totalPrice)),
          new Decimal(0),
        )
        .toFixed(2);
      ...
      this.logger.error('Error in create quotation', error.stack);
    }
  }
  ...
  async findOne(id: string, adminId: string) {
    try {
      ...
      this.logger.error('Error in get single quotation', error.stack);
    }
  }
  ...
  async update(id: string, dto: UpdateQuotationDto, adminId: string) {
    try {
      ...
      this.logger.error('Error in update quotation', error.stack);
    }
  }
  ...
  async remove(id: string, adminId: string) {
    try {
      ...
      this.logger.error('Error in delete quotation', error.stack);
    }
  }
  ...
  async listForAdmin(adminId: string, take = 50, skip = 0) {
    try {
      ...
      this.logger.error('Error in list quotations', error.stack);
    }
  }
  ...
  async publicView(id: string) {
    try {
      ...
      this.logger.error('Error in public view', error.stack);
    }
  }
  ...
  async adminDashboardStats(adminId: string) {
    try {
      ...
      const expired = await this.prisma.quotation.count({
        where: {
          adminId,
          status: 'EXPIRED',
          validityDate: { lte: new Date() },
        },
      });

      return { total, pending, approved, rejected, expired, recent };
    } catch (error) {
      this.logger.error('Error in admin dashboard stats', error.stack);
    }
  }
  ...
  async sendQuotationEmail(id: string, adminId: string) {
    try {
      ...
      this.logger.error('Error in send quotation email', error.stack);
    }
  }
  ...
  async clientRespond(
    ...
    try {
      ...
      this.logger.error('Error in client respond', error.stack);
    }
  }
}
